...
const EditMovie = () => {
    ...
    useEffect(() => {
        ...
        if (id === 0) {
            ...
            fetch(`${process.env.REACT_APP_BACKEND}/genres`, rqOpts)
                .then(rs => rs.json())
                .then(data => {
                    const checks = [];
                    data.forEach(g => {
            ...
            fetch(`${process.env.REACT_APP_BACKEND}/admin/movies/${id}`, rqOpts)
                .then(rs => {
                    if (rs.status !== 200) {
    ...
    const handleSubmit = (event => {
        ...
        fetch(`${process.env.REACT_APP_BACKEND}/admin/movies/${movie.id}`, rqOpts)
            .then(rs => rs.json())
            .then(data => {
                if (data.error) {
